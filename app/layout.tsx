...
async function getEvents() {
  try {
    const eventList = await db.query.events.findMany({
      columns: { id: true, name: true, startTimeDate: true, standbyEnabled: true },
...
export default async function AdminLayout({ children }: AdminLayoutProps) {
  ...
  if (!auth.authorized) {
    if (auth.error === "Not authenticated") {
      const loginUrl = `/api/auth/google?redirect_to=${encodeURIComponent("/")}`;
      return (
        <html>
          <body className={`${inter.variable} ${hedvigLettersSerif.variable} antialiased`}>
            <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-zinc-400 mb-6">Sign in with your Stanford Google account to continue.</p>
                <a
                  href={loginUrl}
